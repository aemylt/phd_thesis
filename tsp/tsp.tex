...
Grover's famous quantum algorithm~\cite{grover96} for fast unstructured search can be applied to the na\"ive classical algorithm to achieve a runtime of $O(\sqrt{n!})$, up to polynomial terms in $n$. However, the best classical algorithms are already substantially faster than this. The Held-Karp algorithm, as discussed in \ref{chp:prelim-q-c}, runs in time $O(n^22^n)$, and its follow-up by Bj{\"o}rklund et al.\ \cite{bjorklund2008} has a better asymptotic runtime in terms of $k$. Other classical algorithms based on branch-and-bound \cite{little1963} or branch-and-cut \cite{padberg1991}, although not analysed asymptotically, are also believed to perform faster.
