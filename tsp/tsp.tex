...
The Xiao-Nagamochi algorithm is described in Algorithm \ref{alg:tsp3}, reproduced from~\cite{xiao2016degree3}.
...
Firstly, we describe the predicate in Algorithm \ref{alg:predicate}.
...
Using the partial assignment $A$, run \FnReduction{$G$, $F$} to get $(G', F')$\;
...
\caption{\label{alg:predicate} The predicate function for the Xiao-Nagamochi algorithm.}
\end{algorithm}
...
The heuristic is described in Algorithm \ref{alg:heuristic}, taking as input a partial assignment $A = ((e_1, A_1),\dots,(e_j, A_j))$ of the edges of $G$.
...
Return an edge in $G$ corresponding to $e_i'$ (if there is more than one, choosing one arbitrarily)\;
}
\caption{\label{alg:heuristic} The heuristic function for the Xiao-Nagamochi algorithm.}
\end{algorithm}
...
The reduction function is described in Algorithm \ref{alg:reduction}, using reductions and procedures from Xiao and Nagamochi \cite{xiao2016degree3}.
...
Run the repeated step again until no further reductions can be applied\;
Return $(G', F')$\;
}
}
\caption{\label{alg:reduction} The Xiao-Nagamochi algorithm for solving the TSP on degree-3 graphs.}
\end{algorithm}
