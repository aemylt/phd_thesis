...
\begin{problem}[Random Circuit Sampling] Let $C$ be a random circuit of depth $O(n)$ which consists of random 1- and 2-qubit gates applied to nearest neighbours on a 2D lattice. Sample from the distribution corresponding to measuring the state $C|0\rangle^{\otimes n}$ in the computational basis.
\end{problem}
...
\begin{figure}
\subfloat[\label{fig:hom-diagram}]{\includegraphics[width=0.45\linewidth]{preliminary_bs/hom}}
\hfill
\subfloat[\label{fig:hom-perfect}]{\includegraphics[width=0.45\linewidth]{preliminary_bs/hom_perfect}}
\caption[Experimental setup of the Hong-Ou-Mandel Dip and probability of different outcomes]{\label{fig:hom}The Hong-Ou-Mandel Dip, a demonstration of two-photon interference. \ref{fig:hom-diagram}: An experimental diagram, consisting of two indistinguishable photons (red circles) in two spatial modes (solid black lines) interacting with a single beam splitter (grey dashed line) before being measured by single photon detectors (white semi-circles). \ref{fig:hom-perfect}: The probability of each outcome from measurement.}
\end{figure}
...
\noindent as the characteristic of $\sigma$ in representation $\lambda$. Note that for any $\sigma' = \tau\sigma\tau$ for some $\tau \in \symm_n$, $\chi_\lambda(\sigma) = \chi_\lambda(\sigma')$.  This means that $\sigma$ and $\sigma'$ are in the same class.
...
So what is known about the computational complexity of these immanants then? The best-known results are for the permanent and determinant, which are $\sharpp$-Hard and computable in polynomial time, respectively \cite{valiant1979, aaronson2011, fisikopoulos2016}. But other hardness results are also known, such as when the Young diagram is of a particular structure known as a hook form, where most of the boxes are in the top row and the remaining rows have at most one box in them \cite{hartmann1985, burgisser2000, brylinski2003, mertens2013}.
